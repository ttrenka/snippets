...
class Storage {
	...
	private function query($sql, $params){
		...
		//record error!
		if($this->last_error && $this->last_error[0] != '' && $this->last_error[0] != '00000') {
			openlog('dblog',LOG_NDELAY,LOG_USER);
			$res = syslog(LOG_WARNING,'SQL Statement Failed ('.$this->last_error[0].'):  '.$sql);
			closelog();
		}
		//return the result
		return $s;
