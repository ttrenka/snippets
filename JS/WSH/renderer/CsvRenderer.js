 ...
 * 	Usage:
 *
 * 	var csv = new CsvRenderer(columns);
 * 	var s = csv.render(JSONArray);
 *
 * 	The variable "s" is a string containing the full output
 * 	of the .csv.
 ************************************************************/

CsvRenderer = function(cols){
	var newline = "\r\n",
		delimiter = ",",
		quote = /\"/g,
		replace = '""';
	...
	function createRowCells(each){
		...
		for(var i=0, l=cols.length; i<l; i++){
			var col = cols[i];
			a.push('"' + ("" + each(col)).replace(quote, replace) + '"');
		}
	...
	function createHeaderCells(){
		...
		for(var i=0, l=cols.length; i<l; i++){
			var col = cols[i];
			a.push('"' + (col.label || col.field).replace(quote, replace) + '"');
		}
		return a.join(delimiter) + newline;
	}
