					...
					if(v !== null){
						...
						if(width >= 1){
							var hl = {x1: width/2, x2: width/2, y1: y - high, y2: y - low},
								op = {x1: 0, x2: (Math.floor(width/2) + Math.floor((finalTheme.series.stroke.width||1)/2)), y1: y-open, y2: y-open},
								cl = {x1: (Math.floor(width/2) - Math.floor((finalTheme.series.stroke.width||1)/2))+1, x2: width, y1: y-close, y2: y-close};
